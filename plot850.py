...
def main(FCNV3_folder,save_figure):
    plt.ioff()  
    ...
    files = 0
    file_list = os.listdir(FCNV3_folder)
    ...
    # data  = nc.Dataset(input_data_path)
    # data  = np.load(input_data_path)

    # for i in range(len(data.variables['lead_time'])):
    for i in range(files):
        print(i)
        data = np.load(os.path.join(FCNV3_folder,f'output_weather_{(i)*6:0>3}h.npy'))
        # print(data.shape)
        ...
        # u = np.flip(data['u850'][0,i, :, :], axis=0)[lat_min:lat_max, lon_min:lon_max]
        # v =  np.flip(data['v850'][0,i, :, :], axis=0)[lat_min:lat_max, lon_min:lon_max]
        # ws = (u**2+v**2)**0.5    
        
        u = np.flip(data[0,17, :, :], axis=0)[lat_min:lat_max, lon_min:lon_max]
        v =  np.flip(data[0,17+13, :, :], axis=0)[lat_min:lat_max, lon_min:lon_max]
        ws = (u**2+v**2)**0.5
...
if __name__ == "__main__":
    ...
    parser.add_argument('-f',"--FCNV3_folder", type=str, required=True)
    parser.add_argument('-o',"--save_figure", type=str, required=True)
