...
def main(IC_data_path, IC_time_str, save_path, FCNV3_weight='FCNV3_weight', fore_hr=60, device="cuda:0"):
  ...
  with torch.no_grad():
      with torch.inference_mode():
          with torch.autocast(device_type="cuda", dtype=torch.float32): 
              ...
              np.save(os.path.join(save_path,f'output_weather_000h.npy'),input_data.cpu().numpy())
              for time_i in range(total_step):
                  ...
                  pred_data = model(input_data, front_step_fore_time, normalized_data=False, replace_state=False)
                  input_data = pred_data
  
                  # save forecast state
                  np.save(os.path.join(save_path,f'output_weather_{(time_i+1)*6:0>3}h.npy'),pred_data.cpu().numpy())
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weight',  help='FCNV3 weight', default='FCNV3_weight')
    parser.add_argument('--fore_hr',  help='forecast hour', default=72)
    parser.add_argument('--device',  help='cpu or cuda', default='cuda:0')
    args = parser.parse_args()  
    
    main(args.input_data, args.input_time, args.output_folder, FCNV3_weight=args.weight, fore_hr=args.fore_hr, device=args.device)  
